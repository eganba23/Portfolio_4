...
```{r visualziation 5, message = FALSE, fig.show="hold", out.width="50%"}
...
```{r visuliation 6, fig.show="hold", out.width="50%"}
...
top_25 %>%
  ggplot(aes(
    x = fct_rev(primary_title),
    y = average_rating
  ))+
  geom_col(alpha = .5)+
  theme_bw()+
  geom_text(aes(x = primary_title, 
              y = average_rating, 
              label = num_votes))+
  coord_flip()+
  labs(
...
bottom_25 <- bottom_25 %>%
mutate(primary_title = case_when(
...
You can see in the side by side comparision that there is more variability in the bottom 25 rating than in the top 25 ratings for movies.